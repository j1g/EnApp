		...
		if (\Input::post('FORM_SUBMIT') == 'tl_upload')
		{
			...
			if ($this->blnIsDbAssisted)
			{
				...
				foreach ($arrUploaded as $strFile)
				{
					...
					//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                     if ($objFile !== null){
					        $tokens = explode('/', $strFile);
							$dir = substr($strFile, 0,strrpos($strFile, '/'));
							$bckpDir = TL_ROOT . '/' . $dir . '/' . 'Backup/';
							if (!file_exists($bckpDir)) {		
								mkdir($bckpDir);
							}
							    
							list($first_part, $second_part) = explode('.', end($tokens));
							$date = date('Y-m-d-H-i-s');
							$newFile = $first_part . "-" . $date . "." . $second_part;
                            $newDir = $bckpDir . $newFile;

							copy(TL_ROOT . '/' . $strFile, $newDir);

							$forDB = $dir . '/Backup/' . $newFile;
							
							\Dbafs::addResource($forDB);
